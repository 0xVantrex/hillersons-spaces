...
import {
  Search, Filter, Grid, List, Eye, ArrowRight,
  ...
  const fetchPlans = async () => {
    try {
      ...
      const normalized = data.map((item) => ({
        ...item,
        images: Array.isArray(item.finalImageURLs) && item.finalImageURLs.length > 0
          ? item.finalImageURLs
    ...
    return products
      .filter((p) => {
        ...
        const matchesCategory =
          activeCategory === "all" ||
          (activeCategory === "Residential" && p.subCategoryGroup?.includes("Residential")) ||
          (activeCategory === "Commercial" && p.subCategoryGroup?.includes("Commercial")) ||
          (activeCategory === "Social" && p.subCategoryGroup?.includes("Social")) ||
          (activeCategory === "Interior" && p.subCategoryGroup?.includes("Interior")) ||
          (activeCategory === "Renovation" && p.subCategoryGroup?.includes("Renovation")) ||
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <Header showSearch={showSearch} searchQuery={searchQuery} setSearchQuery={setSearchQuery}
          ...
          setShowSearch={setShowSearch} projects={projects} />
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <main className="container mx-auto px-4 py-6 pt-24">
        ...
        {/* Mobile filter bar */}
        {isMobile && (
          <div className="sticky top-20 z-40 mb-6 bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center">
            <button
            ...
            <div className="flex bg-gray-100 rounded-lg p-1">
              {["grid", "list"].map((mode) => (
                <button key={mode} onClick={() => setViewMode(mode)}
                  className={`p-2 rounded-md transition ${viewMode === mode ? "bg-white shadow-sm text-emerald-600" : "text-gray-400"}`}>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            {/* Results header */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
              ...
              {!isMobile && (
                <div className="flex items-center gap-3">
                  <div className="flex bg-gray-100 rounded-lg p-1">
                    {["grid", "list"].map((mode) => (
                      <button key={mode} onClick={() => setViewMode(mode)}
                        className={`p-2 rounded-md transition ${viewMode === mode ? "bg-white shadow-sm text-emerald-600" : "text-gray-400 hover:text-gray-600"}`}>
                        {mode === "grid" ? <Grid className="w-4 h-4" /> : <List className="w-4 h-4" />}
                      </button>
                    ))}
                  </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
                ...
                <h3 className="text-xl font-bold text-gray-800 mb-3">No designs found</h3>
            ...
            <div className="mt-12 bg-emerald-600 rounded-2xl p-8 text-white text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <button onClick={() => navigate("/custom-design")}
                  className="bg-white text-emerald-700 hover:bg-emerald-50 px-6 py-3 rounded-xl font-semibold transition flex items-center justify-center gap-2">
                  <Pencil className="w-5 h-5" />
                  Request Custom Design
                </button>
                <a href="mailto:HillersonsDesigns@gmail.com"
                  className="bg-emerald-500 hover:bg-emerald-400 text-white px-6 py-3 rounded-xl font-semibold transition flex items-center justify-center gap-2">
          ...
          <div className="bg-white w-full max-h-[85vh] rounded-t-2xl overflow-hidden">
            ...
            <div className="p-5 overflow-y-auto max-h-[calc(85vh-60px)]">
              ...
              <button onClick={() => setShowFilters(false)}
                className="w-full mt-6 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-semibold transition">
                Apply Filters
              </button>
            </div>
          </div>
